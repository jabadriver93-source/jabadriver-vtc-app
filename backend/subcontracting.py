...
def init_subcontracting(database, stripe_key, admin_email=None, sender_email=None, frontend_url=None):
    ...
    logger.info(f"[SUBCONTRACTING] SENDER_EMAIL: {SENDER_EMAIL or '(not set)'}")
...
async def send_course_assigned_to_client(course: dict, driver: dict):
    ...
    try:
        ...
        logger.info(f"[EMAIL][CLIENT] Sending assignment notification | Course: {course_id_short}")
        
        # Use retry helper for rate limit protection
        result = await send_email_with_retry(params, "[EMAIL][CLIENT]")
        
        if result.get("success"):
            logger.info(f"[EMAIL][CLIENT] ✅ Notification sent | Resend ID: {result.get('resend_id', 'N/A')} | attempts={result.get('attempts', 1)}")
        else:
            logger.error(f"[EMAIL][CLIENT] ❌ Failed | error: {result.get('error', 'Unknown')}")
    except Exception as e:
        logger.error(f"[EMAIL][CLIENT] ❌ Exception | Error: {str(e)}")
        logger.exception("[EMAIL][CLIENT] Full exception trace:")
...
async def token_download_bon_commande(ride_id: str, token: str = Query(...)):
    ...
    return StreamingResponse(
        ...
        headers={"Content-Disposition": f"attachment; filename=bon-commande-{course_id_short}.pdf"}
    )
...
@driver_router.post("/ride/{ride_id}/supplements")
async def token_update_supplements(ride_id: str, token: str = Query(...), data: SupplementsUpdate = None):
    """Update supplements via driver token (no session required)"""
    if not SUBCONTRACTING_ENABLED:
        raise HTTPException(status_code=503, detail="Module sous-traitance désactivé")
    ...
    status = course.get("status")
    invoice_finalized = course.get("invoice_finalized") == True or course.get("invoice_status") == "ISSUED"
    client_confirmed = course.get("client_confirmed") == True or status == "DONE"
    
    logger.info(f"[DRIVER-ACTIONS] Token update supplements | ride={ride_id[:8]} | status={status} | invoice_finalized={invoice_finalized} | client_confirmed={client_confirmed}")
    
    # Rule: Supplements allowed if status in [ASSIGNED, IN_PROGRESS, DRIVER_COMPLETED] AND not locked
    allowed_statuses = [CourseStatusEnum.ASSIGNED, CourseStatusEnum.IN_PROGRESS, CourseStatusEnum.DRIVER_COMPLETED]
    if status not in allowed_statuses:
        raise HTTPException(status_code=400, detail=f"Suppléments non modifiables: statut={status}")
    
    if invoice_finalized:
        raise HTTPException(status_code=400, detail="Facture finale déjà générée - suppléments verrouillés")
...
async def finalize_attribution(course_id: str, driver_id: str, payment_session_id: str):
    ...
    try:
        ...
        email_already_sent = updated_course.get("assignment_email_sent") == True if updated_course else False
        
        logger.info(f"[EMAIL-FLOW][AUTO-ASSIGN] Preparing emails | course={course_id[:8]} | driver_found={bool(driver)} | course_found={bool(updated_course)} | email_already_sent={email_already_sent}")
        
        if email_already_sent:
            logger.info(f"[EMAIL-FLOW][AUTO-ASSIGN] Email already sent for course {course_id[:8]} - skipping to avoid duplicate")
            return True
        
        if driver:
            logger.info(f"[EMAIL-FLOW][AUTO-ASSIGN] Driver details | id={driver.get('id', 'N/A')[:8]} | email={driver.get('email', 'NOT_SET')} | name={driver.get('name', 'N/A')}")
        else:
            logger.error(f"[EMAIL-FLOW][AUTO-ASSIGN] ❌ Driver NOT FOUND in DB | driver_id={driver_id[:8]} | course={course_id[:8]}")
        
        if updated_course:
            logger.info(f"[EMAIL-FLOW][AUTO-ASSIGN] Course details | status={updated_course.get('status')} | driver_access_token={updated_course.get('driver_access_token', 'NONE')[:10] if updated_course.get('driver_access_token') else 'NONE'}...")
        else:
            logger.error(f"[EMAIL-FLOW][AUTO-ASSIGN] ❌ Course NOT FOUND after update | course_id={course_id[:8]}")
        
        # Log config status
        logger.info(f"[EMAIL-FLOW][AUTO-ASSIGN] Config | SENDER_EMAIL={SENDER_EMAIL or 'NOT_SET'} | FRONTEND_URL={FRONTEND_URL or 'NOT_SET'} | RESEND_API_KEY_present={bool(resend.api_key or os.environ.get('RESEND_API_KEY'))}")
            ...
            if email_result and email_result.get("success"):
                await db.courses.update_one(
                    ...
                    {"$set": {
                        ...
                        "assignment_email_sent_at": datetime.now(timezone.utc).isoformat(),
                        "assignment_email_resend_id": email_result.get("resend_id")
                    }}
                )
                logger.info(f"[EMAIL-FLOW][AUTO-ASSIGN] ✅ All 3 emails sent | course={course_id[:8]} | driver_email={driver_email} | resend_id={email_result.get('resend_id')}")
            else:
                logger.error(f"[EMAIL-FLOW][AUTO-ASSIGN] ⚠️ Admin/Client emails sent but driver email FAILED | course={course_id[:8]} | driver_email={driver_email}")
        else:
            logger.error(f"[EMAIL-FLOW][AUTO-ASSIGN] ❌ SKIPPED all emails - driver={bool(driver)} course={bool(updated_course)} | course_id={course_id[:8]}")
    except Exception as e:
        logger.error(f"[EMAIL-FLOW][AUTO-ASSIGN] ❌ Failed to send assignment notifications: {str(e)}")
        logger.exception("[EMAIL-FLOW][AUTO-ASSIGN] Full stack trace:")
...
async def admin_test_driver_email(course_id: str):
    ...
    try:
        ...
        if email_result and email_result.get("success"):
            ...
            sender_used = email_result.get("from", SENDER_EMAIL)
            logger.info(f"[EMAIL-TEST] ✅ Email sent | from={sender_used} | to={driver.get('email')} | resend_id={resend_id}")
            return {
                "success": True,
                ...
                "resend_id": resend_id,
                "note": "Vérifiez ce resend_id dans le dashboard Resend pour voir le statut de livraison."
            }
        else:
